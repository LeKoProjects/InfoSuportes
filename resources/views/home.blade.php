    ...
    <main>
        <section class="border-b border-[#e8f0f8] bg-[radial-gradient(circle_at_20%_20%,#d9ecf7_0,#f8fbfe_30%,#f8fbfe_100%)] transition-colors dark:border-[#1e3a5f] dark:bg-[radial-gradient(circle_at_20%_20%,#1a4b7a_0,#0d1b2e_34%,#0d1b2e_100%)]">
            <div class="mx-auto grid max-w-7xl gap-10 px-5 py-14 md:grid-cols-[1.1fr_0.9fr] md:items-center lg:py-20">
                <div class="max-w-3xl">
                    ...
                    <h1 class="animate-in hero-text font-['Space_Grotesk'] text-4xl font-bold leading-tight tracking-normal md:text-6xl" style="animation-delay: 0.05s">
                        Invista no seu futuro com conhecimento
                    </h1>
                    <p class="animate-in mt-5 max-w-2xl text-lg leading-8 text-[#415149] dark:text-[#b9c7bf]" style="animation-delay: 0.1s">
                        Catálogo curado de livros sobre educação financeira, desenvolvimento pessoal, tecnologia e carreira. Acesse via Hotmart.
                    </p>
                    <div class="animate-in mt-8 flex flex-wrap gap-3" style="animation-delay: 0.15s">
        ...
        <section id="destaques" class="mx-auto max-w-7xl px-5 py-14">
            <div class="mb-7 flex flex-wrap items-end justify-between gap-4">
                <div>
                    <p class="animate-in text-sm font-bold uppercase tracking-normal text-[#1a6fb5]">Destaques</p>
                    <h2 class="animate-in font-['Space_Grotesk'] text-3xl font-bold tracking-normal md:text-4xl" style="animation-delay: 0.05s">Títulos principais</h2>
                </div>
            ...
            <div class="flex snap-x gap-4 overflow-x-auto pb-4">
                @forelse ($featuredBooks as $book)
                    <article class="min-w-[270px] snap-start animate-in rounded-lg border border-[#e8f0f8] bg-white p-4 transition duration-300 hover:shadow-lg hover:border-[#1a6fb5] dark:border-[#1e3a5f] dark:bg-[#1e3a5f] md:min-w-[340px]" style="animation-delay: {{ (0.15 + ($loop->index % 4) * 0.05) }}s">
                        @if ($book->cover_image_url)
                            <img src="{{ $book->cover_image_url }}" alt="Capa do livro {{ $book->title }}" class="aspect-[4/5] w-full rounded-lg object-cover">
                        @else
        ...
        <section id="categorias" class="border-y border-[#e8f0f8] bg-white py-12 dark:border-[#1e3a5f] dark:bg-[#0f2339]">
            <div class="mx-auto max-w-7xl px-5">
                <p class="animate-in text-sm font-bold uppercase tracking-normal text-[#1a6fb5]">Categorias</p>
                <div class="mt-5 flex flex-wrap gap-3">
                    @forelse ($categories as $category)
                        <a href="#catalogo" class="animate-in rounded-lg border border-[#e8f0f8] bg-[#f8fbfe] px-4 py-3 text-sm font-bold text-[#0d1b2e] transition duration-300 hover:border-[#1a6fb5] hover:bg-white dark:border-[#1e3a5f] dark:bg-[#0f1b2a] dark:text-[#eef7f1] dark:hover:bg-[#1e3a5f]" style="animation-delay: {{ $loop->index * 0.08 }}s">
        ...
        <section id="catalogo" class="mx-auto max-w-7xl px-5 py-14">
            <div class="mb-7 max-w-3xl">
                <p class="animate-in text-sm font-bold uppercase tracking-normal text-[#1a6fb5]">Catálogo</p>
                <h2 class="animate-in font-['Space_Grotesk'] text-3xl font-bold tracking-normal md:text-4xl" style="animation-delay: 0.05s">Biblioteca completa</h2>
                <p class="animate-in mt-3 text-[#415149] dark:text-[#b9c7bf]" style="animation-delay: 0.1s">Selecione um título, clique em comprar e finalize seu pedido via Hotmart.</p>
            </div>

            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                @forelse ($books as $book)
                    <article class="group animate-in flex h-full flex-col rounded-lg border border-[#e8f0f8] bg-white p-4 transition duration-300 hover:-translate-y-2 hover:border-[#1a6fb5] hover:shadow-lg dark:border-[#1e3a5f] dark:bg-[#1e3a5f]" style="animation-delay: {{ (0.15 + ($loop->index % 12) * 0.05) }}s">
                        @if ($book->cover_image_url)
                            <img src="{{ $book->cover_image_url }}" alt="Capa do livro {{ $book->title }}" class="aspect-[4/5] w-full rounded-lg object-cover">
                        @else
                            <div class="flex aspect-[4/5] w-full items-center justify-center rounded-lg bg-[#0d1b2e] p-5 text-center font-['Space_Grotesk'] text-2xl font-bold text-white">
        ...
        <section class="bg-[#e8f0f8] py-14 dark:bg-[#0f2339]">
            <div class="mx-auto grid max-w-7xl gap-5 px-5 md:grid-cols-3">
                <div class="animate-in rounded-lg border border-[#b3ddf0] bg-white p-6 transition duration-300 hover:shadow-lg hover:border-[#1a6fb5] dark:border-[#1e3a5f] dark:bg-[#1e3a5f]">
                ...
                <div class="animate-in rounded-lg border border-[#b3ddf0] bg-white p-6 transition duration-300 hover:shadow-lg hover:border-[#1a6fb5] dark:border-[#1e3a5f] dark:bg-[#1e3a5f]" style="animation-delay: 0.1s">
                ...
                <div class="animate-in rounded-lg border border-[#b3ddf0] bg-white p-6 transition duration-300 hover:shadow-lg hover:border-[#1a6fb5] dark:border-[#1e3a5f] dark:bg-[#1e3a5f]" style="animation-delay: 0.2s">
        ...
        <section class="mx-auto max-w-4xl px-5 py-14">
            <p class="animate-in text-sm font-bold uppercase tracking-normal text-[#1a6fb5]">FAQ</p>
            <div class="mt-5 divide-y divide-[#e8f0f8] rounded-lg border border-[#e8f0f8] bg-white dark:divide-[#1e3a5f] dark:border-[#1e3a5f] dark:bg-[#1e3a5f]">
                <details class="animate-in group p-5 transition duration-300 open:bg-blue-50 dark:open:bg-[#0f2339]" open>
                ...
                <details class="animate-in group p-5 transition duration-300 open:bg-blue-50 dark:open:bg-[#0f2339]" style="animation-delay: 0.1s">
                ...
                <details class="animate-in group p-5 transition duration-300 open:bg-blue-50 dark:open:bg-[#0f2339]" style="animation-delay: 0.2s">
